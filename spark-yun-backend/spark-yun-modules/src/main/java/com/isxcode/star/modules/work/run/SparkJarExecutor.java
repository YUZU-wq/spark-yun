...
public class SparkJarExecutor extends WorkExecutor {
  ...
  private final AesUtils aesUtils;
	...
	public SparkJarExecutor(WorkInstanceRepository workInstanceRepository, ClusterRepository clusterRepository,
                          ...
                          WorkRepository workRepository, WorkConfigRepository workConfigRepository, FileRepository fileRepository, Locker locker,
    ...
    SparkJarConfigEntity sparkJarConfigEntity = JSON.parseObject(workConfig.getJarConf(), SparkJarConfigEntity.class);
    ...
    if (sparkJarConfigEntity.getLibFileIds() != null) {
      sparkJarConfigEntity.getLibFileIds().forEach(fileId -> {
		...
		SparkSubmit sparkSubmit = SparkSubmit.builder().verbose(true)
				.mainClass(sparkJarConfigEntity.getMainClass()).appResource(fileEntityOptional.get().getFileName())
				.conf(genSparkSubmitConfig(workRunContext.getSparkConfig())).build();

		// 开始构造executeReq
		executeReq.setSparkSubmit(sparkSubmit);
		executeReq.setPluginReq(JSON.parseObject(Base64.getDecoder().decode(sparkJarConfigEntity.getArgs()), Object.class));
